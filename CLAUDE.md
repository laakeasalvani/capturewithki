...
| Unseen-inquiry alarm | `escalateUnreadInquiries`, hourly. Mails the maintainers if an inquiry is never displayed in the dashboard |
...
5. **Do not break the CMS.** `index.html` carries **131** `data-cms-id` markers
   ...
   before and after any edit to that file with `grep -o data-cms-id index.html | wc -l`
   — `grep -c` counts LINES, not markers, and reads low.
...
## Why the unseen-inquiry alarm exists — read before changing it

On 2026-09-04 Khiara reported she had never received any inquiries. She had:
Gmail was filing every notification into Spam. Nothing in the system could
tell. `submitInquiry` logged `owner email accepted by Resend` for all six
submissions and every one was true — **the Resend API returning 200 only means
the message was queued, and Gmail reports a spam-filed message as delivered.**
Every signal the send path produces was green while she saw nothing.

The lesson: *no signal generated by the sending side can detect this class of
failure.* The alarm therefore watches the only thing that survives it — whether
the inquiry was ever put in front of her.

- `int/inquiries.js` stamps `seenAt` on each inquiry it renders.
- `functions/lib/escalate.js` decides which inquiries are overdue (6 hours,
  with a 7-day ceiling so the back catalogue cannot flood the first run).
- `escalateUnreadInquiries` mails **the maintainers, not
  capturewithki@gmail.com**. An alarm about a broken inbox, delivered to the
  broken inbox, is not an alarm.

Two things that look like simplifications and are not:

- **It must not watch `status`.** That only flips to 'replied' when she presses
  the button, so an inquiry read at 11pm and answered at 9am would raise a
  false alarm every time. An alarm she learns to ignore is worse than none.
- **`seenAt` is pinned to `request.time` in `firestore.rules`,** so it cannot
  be backdated, and the clause is guarded by `affectedKeys` — without that
  guard, every later status or notes edit would be refused for carrying an
  existing `seenAt` that no longer equals `request.time`.

The domain also had **no DMARC record at all** until this was fixed, which is
what made Gmail distrust the mail in the first place. DKIM was already correct.
